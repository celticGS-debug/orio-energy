        ...
        {data.map((item, index) => (
          ...
          >
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full">
              <div className="h-10 absolute left-3 md:left-3 w-10 rounded-full bg-background border-2 border-border flex items-center justify-center shadow-sm">
                <div className="h-4 w-4 rounded-full bg-primary/30 border border-primary/50 p-1" />
              </div>
              <h3 className="hidden md:block text-xl md:pl-20 md:text-4xl font-bold text-primary/60">
                {item.title}
              </h3>
            </div>

            <div className="relative pl-20 pr-4 md:pl-4 w-full">
              <h3 className="md:hidden block text-2xl mb-4 text-left font-bold text-primary/60">
                {item.title}
              </h3>
              {item.content}
            </div>
